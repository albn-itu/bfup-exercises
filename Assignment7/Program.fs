...
let main argv =
    ...
    run (pif .>*>. pthen) "if   then    "
    |> printfn "%A"

    run (pif .>*>. pthen) "    if   then"
    |> printfn "%A"

    run (pif .>*>. pthen .>*>. pelse) "if  then        else"
    |> printfn "%A"

    printfn "\n\nTesting 7.4\n"

    run (parenthesise pint32) "(    5    )"
    |> printfn "%A"

    run (parenthesise pthen) "(    then )"
    |> printfn "%A"

    run (parenthesise pint32) "(    x    )"
    |> printfn "%A"

    run (parenthesise pint32) "(  5    x )"
    |> printfn "%A"
    ...
    run (unop (pchar '-') pint32) "-     5"
    |> printfn "%A"

    run (unop (pchar '-') pint32 |>> (*) (-1)) "-5"
    |> printfn "%A"

    run (unop (pchar '-') pint32) "-     x"
    |> printfn "%A"

    printfn "\n\nTesting 7.7\n"

    run (binop (pchar '+') pint32 pint32) "5 +  7"
    |> printfn "%A"

    run (binop (pchar '+') pint32 pid) "5+var"
    |> printfn "%A"

    run
        (binop (pchar '+') pint32 pint32
         |>> (fun (a, b) -> a + b))
        "5 +  7"
    |> printfn "%A"
    ...
    run AexpParse "pointValue (x % 4) / 0"
    |> printfn "%A"
    ...
    run CexpParse "toLower (toUpper( 'x'))"
    |> printfn "%A"

    run CexpParse "toLower (toUpper 'x')"
    |> printfn "%A"

    run CexpParse "intToChar (charToInt (' '))"
    |> printfn "%A"

    run AexpParse "charToInt (charValue (pointValue (5)))"
    |> printfn "%A"
    ...
    run BexpParse "(5 < 4 /\ 6 <= 3) \/ ~false"
    |> printfn "%A"

    run BexpParse "(5 < 4 \/ 6 <= 3) \/ ~true"
    |> printfn "%A"
    ...
    run stmntParse "declare x; x := 5  ;y:=7"
    |> printfn "%A"

    run stmntParse "if (x < y) then { x := 5 } else { y := 7 }"
    |> printfn "%A"

    run stmntParse "if (x < y) then { x := 5 }"
    |> printfn "%A"

    run stmntParse "while (true) do {x5 := 0} "
    |> printfn "%A"
    ...
    (parseSquareProg tripleWordScore |> Map.find 0) Eval.hello 0 0
    |> printfn "%A"

    (parseSquareProg tripleWordScore |> Map.find 0) Eval.hello 4 0
    |> printfn "%A"

    (parseSquareProg tripleWordScore |> Map.find 0) Eval.hello 0 42
    |> printfn "%A"

    (parseSquareProg tripleWordScore |> Map.find 1) Eval.hello 0 0
    |> printfn "%A"

    (parseSquareProg tripleWordScore |> Map.find 1) Eval.hello 4 0
    |> printfn "%A"

    (parseSquareProg tripleWordScore |> Map.find 1) Eval.hello 0 42
    |> printfn "%A"

    printfn "\n\nTesting 7.13\n"

    let bf =
        parseBoardProg standardBoardSource ((Map.map (fun _ -> parseSquareProg) squares))

    let lookup c i acc =
        bf c
        |> fun (StateMonad.Success sq) ->
            sq
    ...
    let evalSquare w pos acc (m: square) =
        let acc' =
            Map.find 0 m w pos acc
            |> fun (StateMonad.Success x) -> x
        ...
        for x in -10 .. 10 do
            printf
                ...
                (board.squares (x, y)
                 |> (fun (StateMonad.Success sq) ->
                     sq
                     |> Option.map (fun x -> evalSquare Eval.hello 1 2 x |> string))
                 |> Option.defaultValue "#")
