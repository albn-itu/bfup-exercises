...
let main argv =
    ...
    printfn "%A" (lookup "x" |> evalSM state)
    printfn "%A" (push >>>= push >>>= lookup "y" |> evalSM state)
    ...
    printfn "Testing pop:"
    printfn "%A" (push >>>= pop >>>= lookup "x" |> evalSM state)
    printfn "%A" (pop >>>= push >>>= lookup "x" |> evalSM state)
    printfn ""

    printfn "Testing characterValue:"
    printfn "%A" (characterValue 0 |> evalSM state)
    printfn "%A" (characterValue 4 |> evalSM state)
    printfn "%A" (lookup "y" >>= characterValue |> evalSM state)
    printfn ""

    printfn "Testing pointValue:"
    printfn "%A" (pointValue 0 |> evalSM state)
    printfn "%A" (pointValue 4 |> evalSM state)
    printfn "%A" (lookup "y" >>= pointValue |> evalSM state)
    printfn ""

    printfn "Testing add:"
    printfn "%A" (add (ret 5) (ret 7) |> evalSM state)
    printfn "%A" (add (lookup "x") (lookup "y") |> evalSM state)
    printfn "%A" (add wordLength (lookup "z") |> evalSM state)
    printfn ""

    printfn "Testing div:"
    printfn "%A" (div (ret 7) (ret 0) |> evalSM state)
    printfn "%A" (div (ret 7) (ret 5) |> evalSM state)
    printfn "%A" (div (lookup "y") (lookup "x") |> evalSM state)
    printfn "%A" (div wordLength (lookup "z") |> evalSM state)
    printfn ""

    printfn "Testing wordLength:"
    printfn "%A" (wordLength |> evalSM state)
    printfn "%A" (wordLength |> evalSM emptyState)
    printfn ""

    printfn "Testing characterValue:"
    printfn "%A" (characterValue 0 |> evalSM state)
    printfn "%A" (characterValue 4 |> evalSM state)
    printfn "%A" (characterValue 8 |> evalSM state)
    printfn ""

    printfn "Testing pointValue:"
    printfn "%A" (pointValue 0 |> evalSM state)
    printfn "%A" (pointValue 3 |> evalSM state)
    printfn "%A" (pointValue 8 |> evalSM state)
    printfn ""

    printfn "Testing arithEval:"
    printfn "%A" (arithEval (V "x" .+. N 10) |> evalSM state)
    printfn "%A" (arithEval (WL .*. N 10) |> evalSM state)
    printfn "%A" (arithEval (CharToInt(CV(N 0))) |> evalSM state)
    printfn "%A" (arithEval (PV(N -5)) |> evalSM state)
    printfn "%A" (arithEval (V "x" .%. N 0) |> evalSM state)
    printfn ""


    printfn "Testing charEval:"
    printfn "%A" (charEval (C 'H') |> evalSM state)
    printfn "%A" (charEval (ToLower(CV(N 0))) |> evalSM state)
    printfn "%A" (charEval (ToUpper(C 'h')) |> evalSM state)
    printfn "%A" (charEval (CV(V "x" .-. N 1)) |> evalSM state)
    printfn ""


    printfn "Testing boolEval:"
    printfn "%A" (boolEval TT |> evalSM state)
    printfn "%A" (boolEval FF |> evalSM state)
    ...
    printfn "%A" (boolEval (IsVowel(CV(V "x"))) |> evalSM state)
    ...
    printfn
        "%A"
        (boolEval (IsLetter(CV(N 1)))
         |> evalSM (mkState [] [ ('H', 0); ('1', 0) ] []))

    printfn
        "%A"
        (boolEval (IsLetter(CV(N 0)))
         |> evalSM (mkState [] [ ('H', 0); ('1', 0) ] []))

    printfn
        "%A"
        (boolEval (IsVowel(CV(V "x" .-. N 1)))
         |> evalSM state)
    ...
    printfn "%A" (arithEval2 (CharToInt(CV(N 0))) |> evalSM state)
    printfn "%A" (arithEval2 (PV(N -5)) |> evalSM state)
    printfn "%A" (arithEval2 (V "x" .%. N 0) |> evalSM state)
    printfn ""


    printfn "Testing charEval2:"
    printfn "%A" (charEval2 (C 'H') |> evalSM state)
    printfn "%A" (charEval2 (ToLower(CV(N 0))) |> evalSM state)
    printfn "%A" (charEval2 (ToUpper(C 'h')) |> evalSM state)
    printfn "%A" (charEval2 (CV(V "x" .-. N 1)) |> evalSM state)
    printfn ""

    printfn "Testing boolEval2:"
    printfn "%A" (boolEval2 TT |> evalSM state)
    printfn "%A" (boolEval2 FF |> evalSM state)

    printfn
        "%A"
        (boolEval2 ((V "x" .+. V "y") .=. (V "y" .+. V "x"))
         |> evalSM state)
    ...
    printfn "%A" (boolEval2 (IsVowel(CV(V "x"))) |> evalSM state)

    printfn
        "%A"
        (boolEval2 (IsLetter(CV(V "x" .-. N 1)))
         |> evalSM state)
    ...
    printfn
        "%A"
        (stmntEval2 (Ass("x", N 5)) >>>= lookup "x"
         |> evalSM emptyState)

    printfn
        "%A"
        (stmntEval2 (Seq(Declare "x", Ass("x", N 5)))
         >>>= lookup "x"
         |> evalSM emptyState)

    printfn
        "%A"
        (stmntEval2 (Seq(Declare "x", Seq(Declare "y", Seq(Ass("x", WL), Ass("y", N 7)))))
         >>>= lookup "x"
         >>= (fun vx -> lookup "y" >>= (fun vy -> ret (vx, vy)))
         |> evalSM emptyState)
            ...
            Seq(
                ...
                (Seq(
                    ...
                    While(
                        V "i" .<. WL,
                        Seq(
                            ITE(IsVowel(CV(V "i")), Ass("_result_", V "_result_" .*. N -1), Skip),
                            Ass("i", V "i" .+. N 1)
                        )
                    )
                ))
    ...
    let abs v result =
        ITE(v .<. N 0, Ass(result, v .*. N -1), Ass(result, v))
    ...
    let checkSquare f v els =
        ITE(f "xabs" "yabs", Ass("_result_", N v), els)
                        ...
                        checkSquare
                            ...
                            (checkSquare
                                ...
                                (checkSquare
                                    tlsCheck
                                    2
                                    (checkSquare dlsCheck 1 (checkSquare insideCheck 0 (Ass("_result_", N -1))))))
                    )
                )
            )
        )
    ...
    *)
    ...
    for y in -10 .. 10 do
        for x in -10 .. 10 do
            printf
                "%s "
                (stmntToBoardFun standardBoardFun boardMap (x, y)
                 |> evalSquare hello 1 3
                 |> toString)

        printfn ""

    let ids =
        [ (0, stmntSingleLetterScore)
          (1, stmntDoubleLetterScore)
          (2, stmntTripleLetterScore)
          (3, stmntDoubleWordScore)
          (4, stmntTripleWordScore) ]
